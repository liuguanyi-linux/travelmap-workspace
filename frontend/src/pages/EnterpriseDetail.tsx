...
import { Phone, Mail, MessageCircle, MapPin, ArrowLeft, Loader2, Globe } from 'lucide-react';
...
export default function EnterpriseDetail() {
  ...
  useEffect(() => {
    fetch(`/api/enterprises/${id}`)
      ...
      .then(data => { setItem(data); setLoading(false); fetch('/api/enterprises/' + id + '/view', { method: 'POST' }); })
  ...
  const imgUrl = item.image ? getFullImageUrl(item.image) : null;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        {(item.phone || item.wechat || item.kakao || item.email || item.address || item.website) && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
            ...
            {item.address && (
              <div className="flex items-center gap-3 text-gray-800">
                <div className="w-9 h-9 bg-red-50 rounded-full flex items-center justify-center shrink-0"><MapPin size={16} className="text-red-500" /></div>
                <span className="text-sm">{item.address}</span>
              </div>
            )}
            {item.website && (
              <a href={item.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-gray-800 hover:text-blue-600 transition-colors">
                <div className="w-9 h-9 bg-indigo-50 rounded-full flex items-center justify-center shrink-0"><Globe size={16} className="text-indigo-500" /></div>
